module EngineModule
  def generate_offset(key, offset)
    ...
    [((justified_key[0] + justified_key[1]).to_i + offset[-4].to_i) % 28,
     ((justified_key[1] + justified_key[2]).to_i + offset[-3].to_i) % 28,
     ((justified_key[2] + justified_key[3]).to_i + offset[-2].to_i) % 28,
     ((justified_key[3] + justified_key[4]).to_i + offset[-1].to_i) % 28]
  end
